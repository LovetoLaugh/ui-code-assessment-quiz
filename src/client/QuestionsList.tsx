...
class QuestionsList extends React.Component {
  ...
  componentDidMount()
  {
    ...
    .then(response =>{
      ...
      const fetchedQuestions = questionsList.map((qObj:any) => entities.decode(qObj.question));
      const types = questionsList.map((tObj:any)=>tObj.type);
      const correctAns= questionsList.map((caObj:any)=>caObj.correct_answer);

      //incorrect answers array
      let icArray:any =[];
      questionsList.map((icaObj:any)=>{
...
if(!this.state.showResult)
{
    return (
        <div>
          ...
          <p>{this.state.questionsList[this.state.count]}</p>
          </div>

          {this.state.questTypes[this.state.count]==='text'?
            <div>
              <input type="text" className="textBoxStyle" onChange={this.captureAnswer}/>
            </div> :null }

          {this.state.questTypes[this.state.count]==='boolean'?
          <div>
            <label className="radioStyle">
            <input type="radio" name="booleanOption" value="True"  onChange={this.captureAnswer} />True
            </label>
            <label className="radioStyle">
            <input type="radio" name="booleanOption" value="False" onChange={this.captureAnswer} />False
            </label>
          </div>:null}
          ...
          {this.state.questTypes[this.state.count]==='multiple'?
            Object.values(this.state.multipleChoiceAnswers[this.state.count]).map((m:any , index:number) =>{
              return(
                <div key={index}>
                  <label className="radioStyle">
                  <input type="radio" name="mcaOption" value={m} onChange={this.captureAnswer} checked={this.state.selectedAnswer=== m}/>{m}
                  </label>
                </div>
        ...
        else
        {
          return(
            <div className="summaryStyle">
              ...
              <div>
              <button className="restartButtonStyle" onClick={this.restartQuiz}>Restart Quiz</button>
              </div>
            </div>
          );
        }
